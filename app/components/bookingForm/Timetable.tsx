...
function calucalteAvailableTimeSlots(
  ...
  const now = new Date();
  const time = "08:00";
  const availableFromUtz = parse(
    `${date} ${dbAvailablity.fromTime}`,
    "yyyy-MM-dd HH:mm",
    new Date()
  );

  const availableTillUtz = parse(
      ...
      <div className="mt-3 max-h-[300px] overflow-y-auto">
        {availableSlots.length > 0 ? (
          availableSlots.map((slot, index) => {
            const updatedSlot = addMinutes(
              parse(slot, "HH:mm", new Date()),
              -330
            );
            const formattedUpdatedSlot = format(updatedSlot, "HH:mm");
            ...
            return (
              <Link
                href={`?date=${format(
                  selectedDate,
                  "yyyy-MM-dd"
                )}&time=${formattedUpdatedSlot}`}
                key={index}
              >
                <Button className="w-full mb-2" variant="outline">
                  {formattedUpdatedSlot}
                </Button>
              </Link>
            );
          })
